...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for a in legalActions:
            if self.getQValue(state, a) > maxQ:
                ...
                res = set([a])
            elif self.getQValue(state, a) == maxQ:
                res.add(a)

        if len(res) > 0:
            action = random.choice(list(res))
        else:
            action = None
        return action
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        lastAction = self.getAction(nextState)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for ff in f.keys():
            self.weights[ff] += self.alpha * difference * f[ff]
